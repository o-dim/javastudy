...
public class Library {
	...
	private void deleteBook() throws RuntimeException {
		System.out.println("=== 책 삭제하기 ===");
			...
			if(isbn.isEmpty() == false) {
				 //객체 기반의 삭제 (Book 객체간의 비교가 필요하기 때문에 equals를 해야한다.)
				for(Book book : books) {
				...
				for (int i = 0; i < books.size(); i++) {
					...
					if (isbn.equals(book.getIsbn())) {
						Book deletedBook = books.remove(i); // 삭제된 요소(Book)반환
						// Book deleteBook = books.remove(i);
	...
	private void findBook() throws RuntimeException {
		...
		if(books.isEmpty())
			throw new RuntimeException("등록한 책이 없습니다.");
		...
		} throw new RuntimeException(isbn + " isbn 을 가진 책이 없습니다.");
	}
	
	private void printAllBook() throws RuntimeException { //runtimeException은 unchecked-exception이라서 throws RuntimeException을 생략할 수 있
